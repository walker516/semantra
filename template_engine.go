...
func NewBaseTemplateEngine(path string) BaseTemplateEngine {
	return BaseTemplateEngine{templatePath: path}
}
...
func (e *BaseTemplateEngine) RenderTemplate(tmpl string, params map[string]interface{}) (string, error) {
	t, err := template.New("query").Option("missingkey=error").Parse(tmpl)
