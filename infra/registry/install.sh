...
# execute docker run for prepare
docker run --rm \
    -v "$workdir/harbor.yml:/input/harbor.yml" \
    -v "$workdir/data:/data" \
    -v "$workdir:/compose_location" \
    -v "$workdir/common/config:/config" \
    --privileged \
    goharbor/prepare:v2.13.0 prepare $prepare_para

echo ""

if [ -n "$($DOCKER_COMPOSE ps -q 2>/dev/null)" ]; then
